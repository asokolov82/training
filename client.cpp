	...
	while (ind == 0) {
		...
		if (sel == 1) { // вход
			...
			if (strcmp(buffer, "-> login ok") == 0) { // если ответ сервера о успешной авторизации, выводим историю чата
				cout << "Type: 'list' - to display a list of users, 'logout' - to leave the chat  or  'exit' - to close chat" << endl;
				cout << "To send a private message, type symbol ^ after username. Example: Alex^ Good morning" << endl << "----------=Chat history=----------" << endl;
				ind = 1;
				do {
					memset(buffer, 0, sizeof(buffer));
					recv(server, buffer, sizeof(buffer), 0);
...
DWORD WINAPI clientSend(LPVOID lpParam) { //Отправка данных на сервер
	...
	string time_str;
		while (true) {
			...
			if (buffer[0] != 0 && buffer[0] != '\n') { //если не пустое поле и просто нажат enter
				GetSystemTime(&time); // ниже формируем время сообщения
				time_str = to_string(time.wDay) + "." + to_string(time.wMonth) + "." + to_string(time.wYear) + " " + to_string(time.wHour) + "." + to_string(time.wMinute) + "-> ";
				strcpy_s(buffer, (login + ": " + time_str + buffer).c_str()); // добавляем имя пользователя и дату\время к сообщению отправляемому на сервер. переменная login всегда будет содержать имя удачно залогиневшегося
				send(server, buffer, sizeof(buffer), 0);
				memset(buffer, 0, sizeof(buffer));
			}
		}
...
int main() {
	...
	char cip[1024] = { 0 };
	cout << "Enter Server IP or press Enter for localhost): " << endl; 
	fgets(cip, 1024, stdin);
	if (cip[0] != '\n') {
		ip = cip;
		addr.sin_addr.s_addr = inet_addr(ip.c_str());
	} else addr.sin_addr.s_addr = inet_addr("127.0.0.1");
